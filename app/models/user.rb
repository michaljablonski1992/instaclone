class User < ApplicationRecord
  ...
  ## follows
  has_many :waiting_sent_requests, -> { where(accepted: false) }, class_name: 'Follow', foreign_key: 'follower_id', dependent: :destroy
  has_many :follow_requests, -> { where(accepted: false) }, class_name: 'Follow', foreign_key: 'followed_id', dependent: :destroy
  has_many :accepted_recieved_requests, -> { where(accepted: true) }, class_name: 'Follow', foreign_key: 'followed_id', dependent: :destroy
  has_many :accepted_sent_requests, -> { where(accepted: true) }, class_name: 'Follow', foreign_key: 'follower_id', dependent: :destroy
  has_many :followers, through: :accepted_recieved_requests, source: :follower, dependent: :destroy
